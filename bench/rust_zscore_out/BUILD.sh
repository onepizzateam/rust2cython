...
echo "  src/rust_zscore_ffi.rs — do not edit these manually."
...
if [ ! -f "$SCRIPT_DIR/rust_zscore.h" ]; then
    echo "ERROR: rust_zscore.h not found in $SCRIPT_DIR"
    exit 1
fi
...
if python3 -c "import rust_zscore; print('rust_zscore imported successfully')"; then
    echo ""
    echo "SUCCESS. Local build complete."
    echo "Wheel generated in dist/: pip install dist/rust_zscore-*.whl"
else
