...
echo "  src/statrs_ffi.rs — do not edit these manually."
...
if [ ! -f "$SCRIPT_DIR/statrs.h" ]; then
    echo "ERROR: statrs.h not found in $SCRIPT_DIR"
    exit 1
fi
...
if python3 -c "import statrs; print('statrs imported successfully')"; then
    echo ""
    echo "SUCCESS. Local build complete."
    echo "Wheel generated in dist/: pip install dist/statrs-*.whl"
else
