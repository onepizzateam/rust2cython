...
echo "  src/triple_accel_ffi.rs — do not edit these manually."
...
if [ ! -f "$SCRIPT_DIR/triple_accel.h" ]; then
    echo "ERROR: triple_accel.h not found in $SCRIPT_DIR"
    exit 1
fi
...
if python3 -c "import triple_accel; print('triple_accel imported successfully')"; then
    echo ""
    echo "SUCCESS. Local build complete."
    echo "Wheel generated in dist/: pip install dist/triple_accel-*.whl"
else
