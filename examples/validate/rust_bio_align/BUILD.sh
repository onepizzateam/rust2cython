...
echo "  src/rust_bio_align_ffi.rs — do not edit these manually."
...
if [ ! -f "$SCRIPT_DIR/rust_bio_align.h" ]; then
    echo "ERROR: rust_bio_align.h not found in $SCRIPT_DIR"
    exit 1
fi
...
if python3 -c "import rust_bio_align; print('rust_bio_align imported successfully')"; then
    echo ""
    echo "SUCCESS. Local build complete."
    echo "Wheel generated in dist/: pip install dist/rust_bio_align-*.whl"
else
