pub fn generate_build_rs(lib_name: &str, rs_source: Option<&str>, h_source: Option<&str>) -> String {
    // choose input: prefer rs_source
    let input = if let Some(rs) = rs_source { rs } else if let Some(h) = h_source { h } else { "" };

    let mut out = String::new();
    out.push_str("use std::process::Command;\n\n");
    out.push_str("fn main() {\n");
    out.push_str(&format!("    println!(\"cargo:rerun-if-changed={}\");\n", input));
    out.push_str("    let status = Command::new(\"rust2cython\")\n");
    out.push_str(&format!("        .arg(\"{}\")\n", input));
    out.push_str(&format!("        .arg(\"-n\")\n        .arg(\"{}\")\n", lib_name));
