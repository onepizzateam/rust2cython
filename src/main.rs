...
fn main() {
	...
	let name = args.name.clone().unwrap_or_else(|| {
		args.input.file_stem().and_then(|s| s.to_str()).unwrap_or("lib").to_string()
	});

	if args.emit_buildrs {
		let rs_src = if detected == "rust" { args.input.to_str() } else { None };
		let h_src = if detected == "c" { args.input.to_str() } else { None };
	...
	if !args.no_setup {
		...
		println!("Generated {}.pxd, {}.pyx, setup.py, pyproject.toml, BUILD.sh in {}", name, name, args.output.display());
	} else {
		println!("Generated {}.pxd and {}.pyx in {}", name, name, args.output.display());
	}
}
